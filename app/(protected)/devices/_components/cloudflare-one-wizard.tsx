...
import { ArrowLeft, Info, RefreshCw, Star } from "lucide-react"
    ...
    <div className="mx-auto flex min-h-[calc(100svh-8rem)] max-w-3xl flex-col">
          ...
          <WizardSubStep step={2} title="Install the App">
            <div className="mx-auto max-w-sm space-y-4">
              <div className="flex items-start gap-3 rounded-xl border border-border bg-white p-4">
                <div className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-brand-primary text-lg font-bold text-white">
                  CF
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-brand-text-muted">Cloudflare</p>
                  <p className="text-xs text-brand-text-muted">
                    Designed for {platform === "android" ? "tablet" : "iPad"}
                  </p>
                  <div className="mt-1 flex items-center gap-1 text-xs text-brand-text-muted">
                    <Star aria-hidden className="size-3 fill-amber-400 text-amber-400" />
                    4.5 • Free
                  </div>
                </div>
              </div>
              <div className="rounded-xl bg-gradient-to-br from-brand-primary to-indigo-600 p-6 text-center text-white">
                <p className="text-sm font-semibold">
                  The official Cloudflare Zero Trust application
                </p>
              </div>
            </div>
          </WizardSubStep>
      ...
      {currentStep === 2 ? (
        <section className="mt-8 space-y-6">
          ...
          >
            <div className="mx-auto max-w-xs rounded-2xl border border-border bg-white p-4">
              <Label className="text-xs text-brand-text-muted">Enter organization name</Label>
              <div className="mt-2 rounded-lg border border-brand-primary/30 bg-brand-primary/5 px-3 py-2 text-sm font-medium text-brand-text-heading">
                {teamName}
              </div>
            </div>
          </WizardSubStep>
          ...
          <WizardSubStep
            ...
            description={`You'll enter the email ${selectedEmail} since you selected it in the previous step.`}
          >
            <div className="mx-auto max-w-xs space-y-3 rounded-2xl border border-border bg-white p-4">
              <p className="text-center text-sm font-semibold text-brand-text-heading">
                Cloudflare Access
              </p>
              <div className="space-y-1">
                ...
                <div
                  id="setup-email"
                  className="rounded-lg border-2 border-red-400 px-3 py-2 text-sm"
                >
                  {selectedEmail}
                </div>
              </div>
          ...
          >
            <div className="mx-auto max-w-sm rounded-xl border border-border bg-white p-4 text-sm">
              <p className="font-semibold text-brand-text-heading">Cloudflare</p>
              <p className="mt-2 text-brand-text-muted">
                Check your inbox for a one-time login code from Cloudflare and enter
                it in the Cloudflare One app on your device.
              </p>
              <p className="mt-3 rounded-lg border border-dashed border-border bg-brand-surface px-4 py-3 text-sm text-brand-text-muted">
                Login codes are sent to your email and expire quickly. They cannot be
      ...
      {currentStep === 3 ? (
        <section className="mt-8 space-y-6">
          ...
          >
            <div className="mx-auto max-w-md space-y-3 rounded-xl border border-border bg-white p-4 text-center">
              <p className="text-sm text-brand-text-muted">
                Hello <strong className="text-brand-text-heading">3.4.5.6</strong> from
              ...
              <p className="text-xs text-brand-text-muted">What&apos;s the difference?</p>
            </div>
          </WizardSubStep>
          ...
          >
            ...
            <Table>
              ...
              <TableBody>
                <TableRow>
                  <TableCell>{MOCK_DNS_LEAK_RESULT.ip}</TableCell>
                  <TableCell>{MOCK_DNS_LEAK_RESULT.hostname}</TableCell>
                  <TableCell>
                    <span className="rounded border-2 border-red-400 px-2 py-0.5 font-semibold text-brand-primary">
                      {MOCK_DNS_LEAK_RESULT.isp}
                    </span>
                  </TableCell>
                  <TableCell>{MOCK_DNS_LEAK_RESULT.country}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </WizardSubStep>
