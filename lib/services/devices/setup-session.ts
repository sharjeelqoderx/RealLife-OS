...
function emptySession(platform: DevicePlatform): DeviceSetupSession {
  return {
    platform,
...
export async function getDeviceSetupSession(
  ...
  if (error) {
    if (isMissingRelationError(error)) {
      console.warn(
        "getDeviceSetupSession: falling back (table missing — apply tenant_devices migration):",
        error.message
      )
      return emptySession(platform)
    }
...
export async function updateDeviceSetupSession(input: {
  ...
  if (error || !data) {
    if (error && isMissingRelationError(error)) {
      console.warn(
        "updateDeviceSetupSession: falling back (table missing — apply tenant_devices migration):",
        error.message
      )
      return {
        platform: nextPlatform,
        answers: nextAnswers,
        cloudflareWizardStep: nextWizardStep,
